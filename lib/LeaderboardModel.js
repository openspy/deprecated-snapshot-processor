...
LeaderboardModel.prototype.insertOrUpdate = function(update_data) {
    return new Promise(function(resolve, reject) {
        if(!update_data._id) {
            ...
            this.collection.deleteOne({_id: update_data._id}, function(err, result) {
                if(err) return reject(err);
                this.collection.insertOne(update_data, function(err, result) {
                    if(err) return reject(err);
                    resolve();
...
LeaderboardModel.prototype.deleteMany = function(match_data) {
    return new Promise(function(resolve, reject) {
        this.collection.deleteMany(match_data, function(err, results) {
            if(err) return reject(err);
            return resolve();
        });
    }.bind(this));
